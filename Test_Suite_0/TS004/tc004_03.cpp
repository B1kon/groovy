...
int main(int argc, char* argv[]) {
    ...
    const char *ASSERTION="If object Board has object Access,\n"\
			"then PL_CloseBoardDesc returns PL_AGAIN";
    const char *TC="004_03";
    char fnd [256];
    char exp [256];
    int res;
    ...
    __try {// Handler Segmentation Fault
		...
		if((count == 0) || (count == test_cnt)) {
			...
			err_msg(fnd, exp);
			test_res = Fail;
		} else if ((res = PL_CloseBoardDesc(board)) != PL_AGAIN) {
			sprintf_s(fnd, "PL_CloseBoardDesc returns %s %d",
				sp_error_name[res], res);
			sprintf_s(exp, "PL_CloseBoardDesc returns PL_AGAIN");
			err_msg(fnd, exp);
			test_res = Fail;
