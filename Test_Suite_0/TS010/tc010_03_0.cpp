...
int main(int argc, char* argv[]) {
    ...
    const char *not_NM_ELF_code = "tc010_03";
    ...
    __try {// Handler Segmentation Fault
		...
		if((count == 0) || (count == test_cnt)) {
			...
			err_msg(fnd, exp);
			test_res = Fail;
		} else if ((res=PL_LoadProgramFile(access,not_NM_ELF_code)) != PL_FILE){
