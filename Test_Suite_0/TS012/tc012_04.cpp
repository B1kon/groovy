...
int main(int argc, char* argv[]) {
    ...
    const char *ASSERTION="If argument len = 0, "\
			"then no dates are loaded.\n"\
			"PL_ReadMemBlock returns PL_OK.";
    const char *TC="012_04";
    char fnd [256];
    char exp [256];
    int res, i,j;
    ...
    __try {// Handler Segmentation Fault
		...
		for(i=0; i<list; i++){
			...
			if ((res = PL_ReadMemBlock(access, Buffer, AddrStore, 0)) != PL_OK) {
				...
				for(j = 0; j < Length; j++) {
					if(Buffer[j] != 0) {
						sprintf_s(fnd, "Processor %s\n%s received Buffer[%d]=%x",
								processor_list[i],msg, j, Buffer[j]);
						sprintf_s(exp, "Buffer[%d]=0", j);
						err_msg(fnd, exp);
						test_res = Fail;
						break;
					}
