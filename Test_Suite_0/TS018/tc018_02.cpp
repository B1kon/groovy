...
int main(int argc, char* argv[]) {
    ...
    PL_Word protect_array[][2][2] = {
			//PC_array
			{{0x0, 0x7FFFC},{0x100000,0x1FFFFFFF} },
			//CCPU_array
    ...
    __try {// Handler Segmentation Fault
		...
		for(i=0; i<list; i++){
			...
			for (j=0; j<Load_loop[i]; j++) { 
				...
				if ((res=PL_WriteRegister(access,value,Load[i][j][0])) != \
						PL_BADADDRESS) {
					sprintf_s(fnd,"Processor %s addr %x  but\n"\
						" PL_WriteRegister returns %s %d",
						processor_list[i],Load[i][j][0],
						sp_error_name[res], res);
					sprintf_s(exp, "PL_WriteRegister returns PL_BADADDRESS %d",
						PL_BADADDRESS);
					err_msg(fnd, exp);
					test_res = Fail;
					break;
