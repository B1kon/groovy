...
int main(int argc, char* argv[]) {
    ...
    const char *ASSERTION="The result is PL_Write Register = PL_ERROR\n"\
			"if the access argument references NM.";
    const char *TC="018_04";
    char fnd [256];
    char exp [256];
    int res;
    ...
    __try {// Handler Segmentation Fault
		...
		if((count == 0) || (count == test_cnt)) {
			...
			err_msg(fnd, exp);
			test_res = Fail;
		}else if ((res = PL_GetAccess(board, &No_list, &access)) != PL_OK) {
			...
			err_msg(fnd, exp);
			test_res = Fail;
		}else if ((res = PL_WriteRegister(access, value, Load)) != PL_ERROR) {
			sprintf_s(fnd,"Processor %s addr %x  but\n"\
					" PL_WriteRegister returns %s %d",
					processor,Load,	sp_error_name[res], res);
			sprintf_s(exp, "PL_WriteRegister returns PL_ERROR");
			err_msg(fnd, exp);
			test_res = Fail;
		}
    } // End __try
