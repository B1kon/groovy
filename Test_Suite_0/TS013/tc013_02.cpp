...
int main(int argc, char* argv[]) {
    ...
    __try {// Handler Segmentation Fault
		...
		for(i=0; i<2; i++){
			...
			if (res_thr == -100) {
				sprintf_s(fnd, "PL_Sync dosn't wait timeout");
				sprintf_s(exp, "PL_Sync waits timeout");
				err_msg(fnd, exp);
				test_res = Fail;
			}else {
	    ...
	    // should be TIMEOUT
				if (res_thr != -100) {
					sprintf_s(fnd, "PL_Sync steel wait");
					sprintf_s(exp, "PL_Sync returns PL_TIMEOUT");
					err_msg(fnd, exp);
					test_res = Fail;
