...
int main(int argc, char* argv[]) {
    ...
    const char *ASSERTION="If PL_WriteMemBlock returns PL_OK, then data\n" \
			"from area \"block\" by size \"len\" are loaded\n" \
			"into board with starting address \"address\"";
    const char *TC="011_04";
    char fnd [256];
    char exp [256];
    int res;
    PL_Word i,j;
    ...
    #define list	2
    #define rules	2
    const char *prog[] = {"nm_prog_04.abs", "arm_prog_04.elf"};
    const char* processor_list[] = {"NM", "ARM"};
    struct PL_CoreNo No_list[] = {{2,2}, {-1,2}};
    PL_Word Load[][2][2] = {
	// NMMB2_array   for Buffer_fst for Buffer
			{{0x80000,Length_fst},  {0x80001,Length}},
	//CMB2_array
			{{0x20000,Length_fst},  {0x20004,Length}}
			};
    ...
    __try {// Handler Segmentation Fault
		...
		if ((Buffer == (PL_Word *)0) || (Buffer_fst == (PL_Word *)0)){
		...
		bzero((char *)(Buffer_fst), sizeof(PL_Word)*Length_fst);
		Buf[0] = Buffer_fst;
		Buf[1] = Buffer;
